...
class Extra(commands.Cog):
    ...
    async def server(self, ctx):
        if not ctx.guild.banner:
            ...
            embed = discord.Embed(
                ...
                f"[`PNG`]({png}) | [`JPG`]({jpg}) | [`GIF`]({ctx.guild.banner.replace(format='gif')})"
            )
    ...
    async def _user(self, ctx, member: Optional[Union[discord.Member, discord.User]] = None):
        ...
        if not bannerUser.banner:
            await ctx.reply(f"{member} does not have a banner.")
        else:
            ...
            embed = discord.Embed(
                ...
                f"[`PNG`]({png}) | [`JPG`]({jpg}) | [`GIF`]({bannerUser.banner.replace(format='gif')})"
            )
    ...
    async def invite(self, ctx: commands.Context):
        embed = discord.Embed(
            description="> • [Click Here To Invite Nexus To Your Server](https://discord.com/oauth2/authorize?client_id=1096394407823028276&permissions=8&scope=bot)",
            color=0x2f3136
        )
    ...
    @commands.hybrid_command(name="support", aliases=['sup'], description="Join Support Server Of Nexus")
    ...
    async def _vote(self, ctx):
        # button = Button(label="Support", url="https://discord.gg/dwarika")

        embed = discord.Embed(color=discord.Colour(0x2f3136), description="**[Click Here](https://discord.com/invite/dwarika)** To Support Nexus.")
        view = View()
        # view.add_item(button)
        embed.set_author(name='Join Support Server',
    ...
    async def botinfo(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            ...
            description=f"""
            ### <:SageBot:1253676523546742866> About Bot ###
            > **Bot's Mention:** {self.bot.user.mention}
            > **Bot's Username:** {self.bot.user}
            > **Bot's ID:** {self.bot.user.id}
            ### <:NexusStats:1273901502158868504> Discord Stats ###
            > **Total Guilds:** {len(self.bot.guilds)}
            > **Total Users:** {users}
            > **Total Channels:** {channel}
            > **Total Commands:** {len(set(self.bot.walk_commands()))}
            ### <:Nexussystem:1273901601618395187> System Info ###
            ...
            > **Discord.py Version:** `{sys.version_info.micro}.{discord.__version__}`
            """
        )
        embed.set_author(name=f"{ctx.author.name}", icon_url=ctx.author.avatar)
        embed.set_footer(text="</> Made By Team Cosmic !", icon_url=self.bot.user.avatar.url)
        embed.set_thumbnail(url=self.bot.user.avatar.url)
        await ctx.reply(embed=embed)

    @commands.hybrid_command(name="uptime", description="Shows you Bot's Uptime.")
    @blacklist_check()
    @ignore_check()
    async def uptime(self, ctx):
        embed = discord.Embed(
            description=f"<:nexus_rocket:1150313329659752530> | Last Engaged `{str(datetime.timedelta(seconds=int(round(time.time()-start_time))))}` ago.",
            color=self.color
        )
        await ctx.send(embed=embed)
...
**Color:** {member.color if member.color else '000000'}
    ...
    async def status(self, ctx, member: discord.Member = None):
        ...
        await ctx.send(embed=discord.Embed(
            title="**<a:pain_run:1149272995840401428> | status**",
    ...
    async def emoji(self, ctx, emoji: discord.Emoji):
        return await ctx.send(embed=discord.Embed(
            title="**<:nexus_cross_none:1148906686162141256> | emoji**",
            description="emoji: %s\nid: **`%s`**" % (emoji, emoji.id),
            color=0x2f3136))
    ...
    @commands.hybrid_command(name="deleteemoji",
    ...
    async def deleteemoji(self, ctx, emoji: discord.Emoji):
        await emoji.delete()
        await ctx.send(
            "**<:hacker_tick:1271209580793167925> | Successfully emoji has been deleted.**"
        )
    ...
    async def unbanall(self, ctx):
        ...
        button1 = Button(label="No",
                         ...
                         emoji="<:nexus_cross_none:1148906686162141256>")
            ...
            if interaction.user == ctx.author:
                if interaction.guild.me.guild_permissions.ban_members:
                    ...
                    await interaction.channel.send(
                        content=f"<:hacker_tick:1271209580793167925> | Successfully Unbanned {a} Member(s)")
                else:
                    await interaction.response.edit_message(
                        content=
    ...
    async def ping(self, ctx):
        ...
        # Check shard status and latency
        s_id = ctx.guild.shard_id
        shard = self.bot.get_shard(s_id)
        shard_latency = round(shard.latency * 1000, 2)
        shard_uptime_seconds = int(time.time() - psutil.boot_time())
        shard_uptime = str(datetime.timedelta(seconds=shard_uptime_seconds))
        ...
        bot_uptime = {str(datetime.timedelta(seconds=int(round(time.time()-start_time))))}
        ...
        cpu_info = f"{system_info.processor}"

        # Embed description
        description = (
            f"### <:NexusLatency:1273899517825126441> Latency ###\n> API Latency: `{api_latency}` ms\n> Database Latency: `{db_latency}` ms\n> Response Time: `{response_time}` ms\n### <:NexusShards:1273899519913754655> Shard Status ###\n> Shard Latency: `{shard_latency}` ms\n> Shard Uptime: `{shard_uptime}`\n> Shard Status: {'<:icons_online:1271851797975400492> Online' if not shard.is_ws_ratelimited() else '<:Icon_invisible:1271851726730956840> Offline'}"
        )

        embed = discord.Embed(color=self.color, description=description)
        embed.set_author(name=f"Nexus Pong!", icon_url=self.bot.user.display_avatar.url)
    ...
    async def firstmessage(self, ctx):
        ...
        async def fetch_first_message():
            async for message in ctx.channel.history(limit=1, oldest_first=True):
                return message
        first_message = await fetch_first_message()

        button = Button(label="First Message", url = f"{first_message.jump_url}")
        embed = discord.Embed(description=f">>> {first_message.content}", color=0x2f3136)
        embed.set_author(name=first_message.author, icon_url=first_message.author.avatar.url)
    ...
    async def boostlevel(self, ctx):
        server = ctx.guild
        boost_lvl = server.premium_tier

        embed = discord.Embed(title=f"Boost Level Of {server.name}",
                              description=f"`{boost_lvl}`", color=0x2f3136)
        await ctx.send(embed=embed)
    ...
    async def roleicon(self, ctx, role: discord.Role, emoji: discord.PartialEmoji):
        try:
            await role.edit(reason="Role icon updated", icon=str(emoji.url))
            await ctx.send(f'<:hacker_tick:1271209580793167925> | Role "{role.name}" icon has been updated to {emoji}')
        except discord.Forbidden:
            await ctx.send("I don't have the permission to edit the role.")
