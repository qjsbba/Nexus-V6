...
class voice(commands.Cog):
    ...
    async def j2c(self, ctx):
        ...
        embed.set_author(name=f"{ctx.guild.me.display_name}",url="https://discord.gg/atlantisop", icon_url=f"{ctx.guild.me.avatar_url}")
    ...
    @j2c.command()
    async def j2csetup(self, ctx):
        conn = sqlite3.connect('db/voice.db')
        c = conn.cursor()
        guildID = ctx.guild.id
        id = ctx.author.id
        if ctx.author.id == ctx.guild.owner.id or ctx.author.id == 1207080455225213061:
            def check(m):
