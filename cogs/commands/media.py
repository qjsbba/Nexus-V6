...
class Media(commands.Cog):
    ...
    async def on_ready(self):
      with open("db/media.json", "r") as f:
        ok = json.load(f)
      for guild in self.client.guilds:
        if not str(guild.id) in ok:
          ok[str(guild.id)] = {"channel": []}
      with open("db/media.json", "w") as f:
    ...
    async def setup(self, ctx, *, channel: discord.TextChannel):
        ...
        await ctx.send(f"<:hacker_tick:1271209580793167925> | Successfully added {channel.mention} in media only channels.")
    ...
    async def remove(self, ctx, *, channel: discord.TextChannel):
        ...
        await ctx.send(f"<:hacker_tick:1271209580793167925> | Successfully removed {channel.mention} from media only channels.")
    ...
    async def config(self, ctx):
        ...
        embed = discord.Embed(title = f"Media Only Channels for {ctx.guild.name}", color=0x00FFED)
        num = 0
        for i in channel:
            num += 1
            i = i.replace("['']", "")
    ...
    async def reset(self, ctx):
        ...
        await ctx.send("<:hacker_tick:1271209580793167925> | Successfully removed all the media only channels.")
