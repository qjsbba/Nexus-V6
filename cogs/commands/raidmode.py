...
class Raidmode(Cog):
    ...
    async def _antiraid(self, ctx: commands.Context):
        ...
        spam = data["antiSpam"]
        link = data["antiLink"]
        embed = discord.Embed(title="Nexus | Automod Commands",
                              color=0x2f3136)
        embed.add_field(
            name="<:codez_mod:1144609606073602168> antispam on/off",
            value=f"Enables/Disables antispam feature\nCurrently Its {spam}",
            inline=False)
        embed.add_field(
            name="<:codez_mod:1144609606073602168> antilink on/off",
    ...
    async def _antispam(self, ctx: commands.Context, type: str):
        ...
        if ctx.author == ctx.guild.owner or ctx.guild.me.top_role <= ctx.author.top_role:
            if onOroff == "on":
                if data["antiSpam"] is True:
                    hacker = discord.Embed(
                        description=
                        f"<:Nexus_cross:1144687282176147629> | Anti-Spam is already enabled for **`{ctx.guild.name}`**",
                        color=0x2f3136)
                    ...
                    hacker1 = discord.Embed(
                        ...
                        description=
                        f"<:Nexus_tick:1144687280540369018> | Successfully enabled anti-spam for **`{ctx.guild.name}`**",
                        color=0x2f3136)
                ...
                hacker2 = discord.Embed(
                    description=
                    f"<:Nexus_tick:1144687280540369018> | Successfully disabled anti-spam for **`{ctx.guild.name}`**",
                    color=0x2f3136)
                await ctx.reply(embed=hacker2, mention_author=False)
            else:
                hacker3 = discord.Embed(
                    description=
                    f"<:Nexus_cross:1144687282176147629> | Invalid Type.\nIt Should Be On/Off",
                    color=0x2f3136)
            ...
            hacker5 = discord.Embed(
                ...
                description=
                f"<:Nexus_cross:1144687282176147629> | Only owner of the server can run this command"
            )
        ...
        if ctx.author == ctx.guild.owner or ctx.guild.me.top_role <= ctx.author.top_role:
            if onOroff == "on":
                if data["antiLink"] is True:
                    hacker = discord.Embed(
                        description=
                        f"<:Nexus_cross:1144687282176147629> | Anti-link is already enabled for **`{ctx.guild.name}`**",
                        color=0x2f3136)
                    await ctx.reply(embed=hacker, mention_author=False)
                else:
                    data["antiLink"] = True
                    ...
                    hacker1 = discord.Embed(
                        description=
                        f"<:Nexus_tick:1144687280540369018> | Successfully enabled anti-link for **`{ctx.guild.name}`**",
                        color=0x2f3136)
                    await ctx.reply(embed=hacker1, mention_author=False)
                ...
                hacker2 = discord.Embed(
                    description=
                    f"<:Nexus_tick:1144687280540369018> | Successfully disabled anti-link for **`{ctx.guild.name}`**",
                    color=0x2f3136)
                await ctx.reply(embed=hacker2, mention_author=False)
            else:
                hacker3 = discord.Embed(
