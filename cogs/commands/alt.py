...
class AntiAlt(commands.Cog):
    ...
    async def antialt(self, ctx):
        ...
        embed = discord.Embed(description=">>> Navigate through the buttons below to setup and configure **Anti Alt Module** into the guild.", color=0x2F3136)
...
class AntiAltView(discord.ui.View):
    ...
    async def enable(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message("<:hacker_tick:1271209580793167925> **Enabled** Anti-Alt For This Server! ( Default Threshold : 30d )")
    ...
    async def disable(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        saveConfig(self.cog.config)
        await interaction.response.send_message("<:hacker_tick:1271209580793167925> **Disabled** Anti-Alt For This Server!")
...
class PunishmentSelect(discord.ui.View):
    ...
    async def select_punishment(self, interaction: discord.Interaction, select: discord.ui.Select):
        ...
        saveConfig(self.cog.config)
        await interaction.response.send_message(f"<:hacker_tick:1271209580793167925> Punishment type set to **{select.values[0]}** for **Anti-Alt** system.", ephemeral=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if 1 <= days <= 3650:
                ...
                saveConfig(self.cog.config)
                await interaction.response.send_message(f"<:hacker_tick:1271209580793167925> **Anti-Alt** threshold set to **{days}** day(s).", ephemeral=True)
            else:
                await interaction.response.send_message(f"<:hacker_admin_cross:1271209737647558819> **Threshold** must be set between 1 to 3650 days.", ephemeral=True)
        except ValueError:
            await interaction.response.send_message(f"<:hacker_admin_cross:1271209737647558819> Please enter a valid number.", ephemeral=True)
